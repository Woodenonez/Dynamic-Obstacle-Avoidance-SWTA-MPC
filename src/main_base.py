...
class MainBase:
    ...
    def _prepare_interfaces(self, robot:Robot) -> Tuple[MmpInterface, MpcInterface]:
        ...
        return mmp_interface, mpc_interface
    ...
    def run_one_step(self, robot:Robot, human_list:List[Human],
                     tracker_interface: MpcInterface,
                     predictor_interface:MmpInterface=None,
                     verbose:bool=False):
    ...
    def run(self, tracker_type:str, predictor_type:str=None, plot_graph=False):
        ...
        if tracker_type != 'mpc':
            raise ValueError('Tracker type is not supported.')
        if predictor_type not in ['mmp', None]:
            raise ValueError('Predictor type is not supported.')
        
        if self.eval:
            for rep in range(self.max_num_run):
                robot, human_list = self._prepare_agents()
                mmp_intf, mpc_intf = self._prepare_interfaces(robot)
                tracker_intf = mpc_intf
                if predictor_type == 'mmp':
